...
class SubscriptionTest extends FlatSpec with Matchers {
  ...
  it should "not checkpoint a record if the processing function throws" in {
    ...
    val sub = Subscription(Stream(Checkpointable({}, _ => checkpointed = true)).toIterator)
  ...
  //     Subscription(Stream(Checkpointable(0), Checkpointable(1),
  //       Checkpointable(2), Checkpointable(3), Checkpointable(5)))
